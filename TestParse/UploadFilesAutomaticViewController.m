...
- (void)viewDidLoad
{
    ...
    int nbPhotosToUpload = 0;
    for(NSMutableArray *photos in self.photosToUpload){
        nbPhotosToUpload += [photos count];
    }
    
    int nbPhotosAlreadyUploaded = 0;
    for(NSMutableArray *photos in self.photosAlreadyUploaded){
        nbPhotosAlreadyUploaded += [photos count];
    }
    
    NSLog(@"Nombre de photos à uplaoder : %i", nbPhotosToUpload);
    NSLog(@"Nombre DEJA uploadé : %i", [self.photosAlreadyUploaded count]);
    
    [self.inviteButton setBackgroundColor:[MOUtility colorWithHexString:@"fb9f36"]];
}
