...
- (void)removeCell:(PDGestureTableViewCell *)cell completion:(void (^)(void))completion {
    ...
    CGPoint centerX =  cell.frame.origin.x > 0 ? CGPointMake(cell.frame.size.width/2+cell.frame.size.width, cell.center.y) : CGPointMake(-(cell.frame.size.width/2+cell.frame.size.width), cell.center.y);
    
    
    [UIView animateWithDuration:0.3 animations:^{
        [cell setCenter:centerX];
    } completion:^(BOOL finished) {
